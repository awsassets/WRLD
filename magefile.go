...
func Kernel() error {
	...
	detectGoVersion()
	return rundir("app", nil, wrldBin, "build", "-o", "../kernel.elf",
		"-gcflags", GOGCFLAGS,
		"-tags", GOTAGS,
		"./kmain")
}
...
func Multiboot() error {
	...
	return sh.Copy(
		filepath.Join("cmd", "wrld", "assets", "boot", "multiboot.elf"),
		"multiboot.elf",
	)
}
...
func Test() error {
	...
	return rundir("tests", envs, wrldBin, "test")
}
...
func Qemu() error {
	...
	detectQemu()
	return wrldrun(QEMU_OPT, "kernel.elf")
}
...
func QemuDebug() error {
	...
	detectQemu()
	return wrldrun(QEMU_DEBUG_OPT, "kernel.elf")
}

func Iso() error {
	mg.Deps(Kernel)
	return sh.RunV(wrldBin, "pack", "-o", "wrld.iso", "-k", "kernel.elf")
}
...
func Graphic() error {
	...
	mg.Deps(Iso)
	return wrldrun(QEMU_OPT, "wrld.iso")
}
...
func GraphicDebug() error {
	...
	GOGCFLAGS += " -N -l"
	mg.Deps(Iso)
	return wrldrun(QEMU_DEBUG_OPT, "wrld.iso")
}

func WRLD() error {
	err := rundir("cmd", nil, "go", "build", "-o", "../wrld", "./wrld")
	if err != nil {
		return err
	}
	current, _ := os.Getwd()
	wrldBin = filepath.Join(current, "wrld")
	return nil
}
...
func wrldrun(qemuArgs []string, flags ...string) error {
	...
	return sh.RunWithV(envs, wrldBin, args...)
}
